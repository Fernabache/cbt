...
?>
<h3 class="hhh">SET EXAMINATION TIMEOUT IN MINUTES</h3>
<?php
if(isset($_POST['timer_minutes']) && !empty($_POST['timer_minutes'])){
require_once("config.php");

$tok = htmlentities($_POST['timer_minutes'] ,ENT_QUOTES);
...
$msg="<script type='text/javascript'>alert('Exam timeout is now set to $time minutes')</script>";
echo $msg;
}
...
if($msql){
if($t_mill == 1){
$msg="<script type='text/javascript'>alert('Exam timeout is now set to $time minute')</script>";
echo $msg;
}
else{
$msg="<script type='text/javascript'>alert('Exam timeout is now set to $time minutes')</script>";
...
<tr>
<td><input type="hidden" name="timer_minutes" value="<?php $ra = "wolexzo07dacrackeristhebossofcodehouse";$raa = sha1(uniqid($ra));echo $raa;?>" /></td>
