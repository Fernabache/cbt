...
$second = value(replace_wolex($open_tag ,$close_tag , $_POST["second"]));
$third = value(replace_wolex($open_tag ,$close_tag , $_POST["third"]));
$fourth = value(replace_wolex($open_tag ,$close_tag , $_POST["fourth"]));
...
header("location:question_post.php?msg=$msg&token=$tk");
exit;
}
$sqlcommand = "INSERT INTO questions (paper_type ,categories , exam_type ,question ,f_option ,s_option ,t_option ,ft_option ,answer , session , semester ,examiner ,ip_address , OS , browser ,date_time , token) VALUES('$qtype', '$cat' , '$cattype' , '$post' ,'$first ', '$second' ,'$third' ,'$fourth' ,'$answer' , '$session' , '$semester' ,'$exam' ,'$ip' ,'$os' ,'$browser' , now() ,'$token')";
