...
$pager= $_POST["cnum"];
$db_acct = "SELECT * FROM portal WHERE id='1'";
...
header("location:exams.php?pn=$pager&msg=$msg");
exit;
}
...
$etype = clean_data($_POST['type']);
$cat = clean_data($_POST['cat']);
$pager= $_POST["cnum"];
...
$arr = clean_data(strtolower(implode($post ,"-")));
...
$correct_ans = clean_data(strtolower($row["answer"]));
$m_ans = explode(",",clean_data(strtolower($row["answer"])));

if($count == 1){

$db_acct = "SELECT * FROM option_update WHERE id='1'";
$qu_acct = mysql_query($db_acct);
$num_acct = mysql_num_rows($qu_acct);
$fet_acct = mysql_fetch_array($qu_acct);
...
header("location:exams.php?pn=$pager");
}
