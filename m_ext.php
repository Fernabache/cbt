...
<form method="POST" action='m_process.php' name="datareader" autocomplete="off">
...
$msg_sele = "SELECT * FROM exams_scores WHERE attempted_num='$id' AND script_owner='$owner' ";
if(!$mq=mysql_query($msg_sele)){
echo "ERROR :unable to query db";
}
else{
$mq=mysql_query($msg_sele);
$qnum = mysql_num_rows($mq);
if($qnum == 0){

}
else{
$rot = mysql_fetch_array($mq);
$ans = $rot["answer"];
$fc = $rot["final_comment"];

$db_acct = "SELECT * FROM allow_status WHERE id='1'";
